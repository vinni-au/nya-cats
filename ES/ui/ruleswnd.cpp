    ...
    QMainWindow(parent)
{
    ...
    this->retranslate();
    //модели
    this->lbListRules->setModel(DataModels::instance()->ruleModel);
...
RulesWnd::onCurrentRuleChanged(QModelIndex index)
{
    ...
    RuleModel *rm=DataModels::instance()->ruleModel;
...
RulesWnd::onAddRuleClick()
{
    ...
    QModelIndex selRuleIndex = DataModels::instance()->ruleModel->insertRule(lbListRules->currentIndex());
...
RulesWnd::onDeleteRuleClick()
{
    ...
    DataModels::instance()->ruleModel->deleteRule(index);
}
...
RulesWnd::onAddIfClick()
{
    ...
    do
    {
        if(exprEditor.exec())
        {
            ...
            QModelIndex index = DataModels::instance()->ruleModel->addIf(ruleIndex);
            Expr *expr = exprEditor.getExpr();
            DataModels::instance()->ruleModel->setData(index,(uint)expr,Qt::EditRole);
        }
    }
...
RulesWnd::onEditIfClick()
{
    ...
    QString sVar = DataModels::instance()->ruleModel->getExprPart(ifIndex,RuleModel::ExprVar);
    QString sSign = DataModels::instance()->ruleModel->getExprPart(ifIndex,RuleModel::ExprSign);
    QString sVal = DataModels::instance()->ruleModel->getExprPart(ifIndex,RuleModel::ExprVal);
    ...
    {
        Expr *expr = exprEditor.getExpr();
        DataModels::instance()->ruleModel->setData(ifIndex,(uint)expr,Qt::EditRole);
    }
}
...
RulesWnd::onDelIfClick()
{
    ...
    QModelIndex ifIndex = lbIf->currentIndex();
    DataModels::instance()->ruleModel->deleteIf(ifIndex);
}
...
RulesWnd::onAddThenClick()
{
    ...
    do
    {
        ...
        {
            QModelIndex ruleIndex = lbListRules->currentIndex();
            QModelIndex index = DataModels::instance()->ruleModel->addThen(ruleIndex);
            Expr *expr = exprEditor.getExpr();
            DataModels::instance()->ruleModel->setData(index,(uint)expr,Qt::EditRole);
        }
    }
...
RulesWnd::onEditThenClick()
{
    ...
    //получаем выражение по частям
    QString sVar = DataModels::instance()->ruleModel->getExprPart(thenIndex,RuleModel::ExprVar);
    QString sSign = DataModels::instance()->ruleModel->getExprPart(thenIndex,RuleModel::ExprSign);
    QString sVal = DataModels::instance()->ruleModel->getExprPart(thenIndex,RuleModel::ExprVal);
    ...
    {
        Expr *expr = exprEditor.getExpr();
        DataModels::instance()->ruleModel->setData(thenIndex,(uint)expr,Qt::EditRole);
    }
}
...
RulesWnd::onDelThenClick()
{
    ...
    QModelIndex thenIndex = lbThen->currentIndex();
    DataModels::instance()->ruleModel->deleteThen(thenIndex);
}
...
{
    QModelIndex firstRuleIndex = DataModels::instance()->ruleModel->index(0,0,QModelIndex());
    lbListRules->selectionModel()->setCurrentIndex(firstRuleIndex,QItemSelectionModel::ClearAndSelect);

    QModelIndex iIfHead = DataModels::instance()->ruleModel->index(1,0,firstRuleIndex);
    QModelIndex firstIfInx = DataModels::instance()->ruleModel->index(0,0,iIfHead);
    if(firstIfInx.isValid())
        lbIf->selectionModel()->setCurrentIndex(firstIfInx,QItemSelectionModel::ClearAndSelect);

    QModelIndex iThenHead = DataModels::instance()->ruleModel->index(2,0,firstRuleIndex);
    QModelIndex firstThenInx = DataModels::instance()->ruleModel->index(0,0,iThenHead);
